...
been studying computational blending \cite{c1}, and CAs seem to do a
...
Following the approach of Goguen \cite{c2}, we propose exploiting
...
\item[Blend] is the space computed by combining both theories. The computation is computed using a ``pushout'' from the underlying categorical semantics \cite{c3}. 
\end{description}
...
provided by HETS. However, as discussed in \S\ref{fur}, future work
...
\subsubsection{Localised propagation}
...
partitions on a finite length line (0-8), and correspond to the {\em
  genotype} describe in \S\ref{Secno}:
\begin{center}
...
\subsubsection{Modified Genotypes}

As discussed in \S\ref{blend}, it is possible to exploit existing machinery to calculate blends of genotypes which are expressed in very different languages. As an example consider....

\subsubsection{Social Interpretations}



\subsubsection{Implementaion}
The genotype can again be represented in CASL as shown in Figure \ref{fig:casl2d}, with HETS performing a
pushout to create the necessary blend. 
\begin{figure}[!ht]
CASL theory GOES HERE
\label{fig:casl2d}
\caption{A CASL specification for a 2d genotype}
\end{figure}
...
and the code is available\footnote{\url{https://github.com/ewenmaclean/2dCA}}.
